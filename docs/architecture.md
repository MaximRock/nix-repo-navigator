...
- **Nix:** `lexer.py` (IMPL, PATH) → `parser.py` (recursive descent, 21 goldens, UnresolvedExpr) → `ast_extract.py` (imports incl. bare `import ./x` + `modules = [ … ]` path literals, ModuleRef selects, options/config/mkIf/mkMerge/specialisation/_module.args/home.file/packages) → `module_parser.py` (RawNode/RawEdge, ids `nix:*`, `nix_option:*`, `file:*`, `package:*`, `flake_input:*`) → `nix_parser.py` (orchestrator, fallback >50% Unresolved).
